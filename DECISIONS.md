...
**Decision:** Adopt an explicit primary user and a named policy anchor for the project's framing, off the back of a strategic-system review and four commissioned deep-research passes (PM-recruiting reality, adjacent-role pathways, this project's own user and problem, and a product-metrics curriculum). The canonical user is a digital-trade policy-research analyst (concretely the advisor, in the GTAP-Purdue, NCAER and ICRIER orbit), whose recurring decision is "when a specific digital-trade policy is in play, which MSME-relevant service sub-sectors are most exposed, so I know which to commission a deep-dive on or cite in a submission." The WTO e-commerce moratorium (in play at MC14) becomes the first named policy the exposure narrative is framed against, giving a concrete answer to "exposure to what."

**Why:** The research was blunt that a portfolio project lives or dies on a real, defensible user and a narrow problem, and that the firm-level "exposure index" some earlier framings implied is not data-defensible: no public dataset joins an Indian MSME's sector to its digitally-delivered-services trade. Naming the analyst-user and a live policy turns an abstract "exposure" score into a decision someone actually makes, and it matches what the page already does well (rank sectors as direction, surface the crosswalk, show the open methodology questions).

**What this does and does not change:** This is a framing and direction decision, not a rebuild. v0's computed exposure stays policy-agnostic (MSME scale plus Mode 1 trade, the existing two-component score); the policy-sensitivity component (OECD Digital STRI) and the scenario view that would re-rank against named levers remain deferred to v2 per the roadmap. The honest-heuristic framing already on the page (read as direction not magnitude, the EBOPS-to-ISIC crosswalk labelled as a constructed link) is affirmed as the right call, not altered.
